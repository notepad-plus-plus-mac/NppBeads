console.log('[NppBeads] viewer.js: entering top');
window.__nppViewerReached = { top: true };
/**
...
async function initSqlJs() {
  ...
  // Load sql.js. Upstream path: dynamically append a <script> and await
  // onload/onerror. Under WKWebView on file:// those events silently
  // never fire, so we hang. NppBeads preloads sql-wasm.js via a STATIC
  // <script> in index.html and stashes its initSqlJs as window.__nppSqlJs
  // (the `async function initSqlJs()` below would otherwise clobber
  // window.initSqlJs and cause infinite recursion).
  const sqlPromise = initSqlJs.cached || (initSqlJs.cached = new Promise(async (resolve, reject) => {
    try {
      const sqlJs = window.__nppSqlJs;
      if (typeof sqlJs !== 'function') {
        throw new Error('sql-wasm.js was not preloaded by NppBeads');
      }

      // nppbeads:// scheme handler serves vendor/sql-wasm.wasm with the
      // correct application/wasm MIME, so instantiateStreaming succeeds —
      // no need to force wasmBinary from a preloaded blob.
...
window.__nppViewerReached.bottom = true;
console.log('[NppBeads] viewer.js: reached bottom, window.beadsViewer exported');
